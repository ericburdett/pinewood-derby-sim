...
// WI12 — local garage: save/list/load/delete/clear (PRD AC-S1, AC-S2, AC-S3).
...
test("a design saves, persists across reload, and reloading reproduces its time (AC-S1, AC-S2)", async ({
  ...
  // Move the controls away, then load — every control is restored (AC-S2).
...
test("loading a saved car refreshes the preview and inspection, not just the sliders", async ({
  ...
  await expect(page.locator("#preview-summary")).toContainText("sleek wedge");
  ...
  await expect(page.locator("#preview-summary")).toContainText("blocky");
